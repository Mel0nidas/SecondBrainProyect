...
as the model, a **RAG** pipeline (Chroma + Voyage embeddings) so it can
answer from your own notes, and an evaluation harness that gates changes
to the router.
...
| *"what had I saved about webhooks?"* | `consultar` | The **Bibliotecario** (Sonnet, **read-only**) does **RAG**: semantic search over the vault (Chroma + Voyage embeddings) retrieves the relevant notes, Claude answers grounded in them, and the reply ends with `Fuentes: [[note]]` so you can jump to the source in Obsidian — it finds notes even if you used different words. |
...
| RAG / semantic search | Chroma (embedded, persisted to disk) + Voyage AI embeddings (`voyage-3.5-lite`); retrieved fragments are cited back as `[[note]]` |
...
Phases 0–16 are done and running in production 24/7, including RAG with
source citations for the Bibliotecario and real cost tracking for
`/costos`. Known gaps and next steps are tracked in `DISEÑO.md` (roadmap
section) — notably Google Calendar integration.
